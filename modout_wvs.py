...
grabnum = re.compile(r"wvs_(\d+)_(\d+)_(\d+).*") ## change this to reflect adding dataset for water vapor scale (wvs)
...
if __name__ == "__main__":
    keep_dict = {}
    files = list(Path().glob("wvs*.txt"))
    wvs_d1 = [grabnum.search(str(item)).group(1) for item in files] 
    wvs_d2 = [grabnum.search(str(item)).group(2) for item in files] 
    toc_dict={}
    k = 0
    wvs_full = [None]*len(wvs_d1) ## wvs_d1 should be the same size as wvs_d2
    # initialise the dictionary with it's keys here
    for i, j in zip(wvs_d1, wvs_d2):
            wvs_full[k] = '{}.{}'.format(i,j)
            k=k+1
    toc_dict = dict.fromkeys(wvs_full)
    ...
    for the_file, the_dir in zip(files, out_dirs):
        ...
        toc_dict[wvs_full[k]] = str(the_dir)
        f"created {the_dir}"
        ...
        k=k+1
    with open('toc_files_wvs.json', 'w') as outfile:
